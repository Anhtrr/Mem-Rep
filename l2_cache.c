...
void l2_initialize()
{
  ...
  // for each set in L2
  for (int i = 0; i < L2_NUM_CACHE_SETS; i++){ 
    // for each entry in set i 
    for (int j = 0; j < L2_LINES_PER_SET; j++){ 
       l2_cache[i].lines[j].v_r_d_tag = 0; 
    }
  }
}
		     ...
		     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  //bits the appropriate amount.
  uint32_t address_tag = (address & L2_ADDRESS_TAG_MASK)>>L2_ADDRESS_TAG_SHIFT;
  ...
  // if cache hit 
  for (int i = 0; i < L2_LINES_PER_SET; i++){
    uint32_t vrdtag = l2_cache[set_index].lines[i].v_r_d_tag;
    
    // if valid bit is set
    if (vrdtag & L2_VBIT_MASK){ 
      
      // if tag matches 
      if (address_tag == (vrdtag & L2_ENTRY_TAG_MASK)){ 
        
        // set R-bit 
        l2_cache[set_index].lines[i].v_r_d_tag |= L2_RBIT_MASK; 
        
        // set status lowest bit 
        *status |= L2_HIT_STATUS_MASK; 
        
        // preserve line index with cache hit
        line_index = i;  
        break; 
      }
    }
  ...
  if(control & READ_ENABLE_MASK){ // if read-enable bit of control parameter is 1 
		    ...
		    uint8_t *status)
{
  ...
  uint32_t address_set_index = (address & L2_ADDRESS_INDEX_MASK)>>L2_ADDRESS_INDEX_SHIFT; 
  ...
  for (int i = 0; i < L2_LINES_PER_SET; i++){ // each line in set 
    ...
    uint32_t vrdtag = l2_cache[address_set_index].lines[i].v_r_d_tag;
    
    // if valid bit is 0
    if ((vrdtag & L2_VBIT_MASK)==0){ 
      
      // overwrite cache line in the entry 
      for (int j = 0; j < WORDS_PER_CACHE_LINE; j++){
        l2_cache[address_set_index].lines[i].cache_line[j] =  write_data[j];
      }
      
      // write address tag into entry 
      vrdtag = address_tag; 
      ...
      // zero out dirty bit
      vrdtag &= ~(L2_DIRTYBIT_MASK); 

      // zero out reference bit
      vrdtag &= ~(L2_RBIT_MASK); 

      //write new vrdtag back to entry 
      l2_cache[address_set_index].lines[i].v_r_d_tag = vrdtag; 

      // zero out low bit of status 
      *status &= ~(0x1); 
    ...
    // if valid bit is 1
      ...
      if ((vrdtag & L2_RBIT_MASK) == 0 &&(vrdtag & L2_DIRTYBIT_MASK) == 0){
        if(r0_d0_index == NOT_FOUND){
          r0_d0_index = i; // i = line index 
        }
      }
      if ((vrdtag & L2_RBIT_MASK) == 0 && (vrdtag & L2_DIRTYBIT_MASK)){
        if(r0_d1_index == NOT_FOUND){
          r0_d1_index = i;
        }
      }
      if ((vrdtag & L2_RBIT_MASK) && (vrdtag & L2_DIRTYBIT_MASK) == 0){
  ...
  uint32_t evict_entry;
  
  // choosing the entry to evict based on priority 
  if (r0_d0_index == NOT_FOUND){
    if (r0_d1_index == NOT_FOUND){
      // if none of the preferences are available (aka r=1 d=1 v=1)
      if (r1_d0_index == NOT_FOUND){
        evict_entry = 0;
      }
      else{
        evict_entry = r1_d0_index;
      }
    }
    else{
      evict_entry = r0_d1_index;
    }
  }
  else{
    evict_entry = r0_d0_index;
  }
  ...
  uint32_t evicted_entry_tag = l2_cache[address_set_index].lines[evict_entry].v_r_d_tag;
  
  // if dirty bit is set
  if (evicted_entry_tag & L2_DIRTYBIT_MASK){ 
    
    // construct address to write current entry back
    uint32_t addie = ((evicted_entry_tag & L2_ENTRY_TAG_MASK) << L2_ADDRESS_TAG_SHIFT) | (address_set_index << L2_ADDRESS_INDEX_SHIFT); 
    
    // address written to evicted_writeback_address
    *evicted_writeback_address = addie;  
    
    // write data to evicted data array
    for (int i = 0; i < WORDS_PER_CACHE_LINE; i++){
      evicted_writeback_data[i] = l2_cache[address_set_index].lines[evict_entry].cache_line[i]; 
    }

    // set low bit of status to 1 
    *status |= (0x1); 
  }
  ...
  if ((evicted_entry_tag & L2_DIRTYBIT_MASK)== 0){
    *status &= ~(0x1); // zero out low bit
  }
  ...
  //the entry.
  for (int i = 0; i < WORDS_PER_CACHE_LINE; i++){
    l2_cache[address_set_index].lines[evict_entry].cache_line[i]=write_data[i];
  }
  l2_cache[address_set_index].lines[evict_entry].v_r_d_tag = address_tag;
  l2_cache[address_set_index].lines[evict_entry].v_r_d_tag |= L2_VBIT_MASK; 
  l2_cache[address_set_index].lines[evict_entry].v_r_d_tag &= ~(L2_DIRTYBIT_MASK); 
}
...
void l2_clear_r_bits()
{
  ...
  for (int i = 0; i < L2_NUM_CACHE_SETS; i++){ 
    ...
    for (int j = 0; j < L2_LINES_PER_SET; j++){ 
       ...
       // clear out rbit 
       l2_cache[i].lines[j].v_r_d_tag &= ~(L2_RBIT_MASK); 
    }
  }
}
