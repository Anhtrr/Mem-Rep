...
void memory_handle_l1_miss(uint32_t address)  
{
  ...
  uint32_t read_data[WORDS_PER_CACHE_LINE]; // array to fetch data from l2
  uint8_t status;
  ...
  uint8_t status2;
  l1_insert_line(address, read_data, &evicted_writeback_address, evicted_writeback_data, &status2);
  ...
  uint8_t status3;
  // if writeback of evicted entry is required
  if (status2 & (0x1)){
    l2_cache_access(evicted_writeback_address, evicted_writeback_data, WRITE_ENABLE_MASK, NULL, &status3);
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  if (control & WRITE_ENABLE_MASK){
    main_memory_access(address, cache_line, control, NULL);
  }
  ...
  uint32_t evicted_addie;
  uint32_t evicted_data[WORDS_PER_CACHE_LINE];
  uint8_t status; 

  l2_insert_line(address, cache_line, &evicted_addie, evicted_data, &status);
  ...
  if (status & (0x1)){
    main_memory_access(evicted_addie, evicted_data, WRITE_ENABLE_MASK, NULL);
  }
