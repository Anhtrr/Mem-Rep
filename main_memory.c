...
void main_memory_initialize(uint32_t size_in_bytes)
{
  ...
  for (uint32_t i = 0; i < (size_in_bytes>>2); i++){
    main_memory[i] = 0;
  }
...

{
  ...
  if ((address < 0)|| (address >= main_memory_size_in_bytes)){
    printf("Error: Specified address must be within the size of memory (in bytes).\n");
    exit(1);
  }
  ...
  if (control & READ_ENABLE_MASK){
    uint32_t iterate = (cache_line_address>>2);
    for (int i = 0; i < WORDS_PER_CACHE_LINE; i++){
      read_data[i] = main_memory[iterate];
      iterate++;
    }
  }
  ...
  //main memory split into words, each index is a word (4 bytes)
  if (control & WRITE_ENABLE_MASK){
    uint32_t iterate2 = (cache_line_address>>2);
    for (int i = 0; i < WORDS_PER_CACHE_LINE; i++){
      main_memory[iterate2] = write_data[i];
      iterate2++;
    }
  }
}
