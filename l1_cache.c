...
void l1_initialize()
{
  ...
  for (int i = 0; i < L1_NUM_CACHE_ENTRIES; i++){ 
    // set entire v_d_tag as 0
    l1_cache[i].v_d_tag = 0; 
  }
}
		     ...
		     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  uint32_t address_tag = (address & L1_ADDRESS_TAG_MASK) >> L1_ADDRESS_TAG_SHIFT;
  ...
  // if cache miss,
  // zero out lowest bit of status and return 
  uint32_t vdtag = l1_cache[entry_index].v_d_tag; 
  if ((vdtag & L1_VBIT_MASK)==0 || (vdtag & L1_ENTRY_TAG_MASK) != address_tag){
    *status &= ~(L1_HIT_STATUS_MASK);   
    return; 
  }
  ...
  *status |= L1_HIT_STATUS_MASK;
  ...
  if (control & WRITE_ENABLE_MASK){
    // value of write data written to entry's cache line at word index 
    l1_cache[entry_index].cache_line[word_offset] = write_data; 
    // set dirty bit of entry 
		    ...
		    uint8_t *status)
{
  ...
  uint32_t vdtag = l1_cache[entry_index].v_d_tag;
  if ((vdtag & L1_VBIT_MASK) && (vdtag & L1_DIRTYBIT_MASK)){
    // only take the tag bits of entry 
    uint32_t evicted_tag = (vdtag & L1_ENTRY_TAG_MASK);
    // construct new evicted address 
    uint32_t addie = (evicted_tag << L1_ADDRESS_TAG_SHIFT) | (entry_index << L1_ADDRESS_INDEX_SHIFT);
    // write addie to output evicted address
    *evicted_writeback_address = addie; 

    // write cache line data to array
    for (int i = 0; i < WORDS_PER_CACHE_LINE; i++){
      evicted_writeback_data[i] = l1_cache[entry_index].cache_line[i];
    }

    // set lowest bit of status byte to 1 
    *status |= (0x1); 
  }
  ...
  // write the tag
  l1_cache[entry_index].v_d_tag = address_tag;
  // set valid bit
